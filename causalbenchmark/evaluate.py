...
def _score_population(predictions_location, cf_dir_location):
    ...
    ufids = [f.rsplit(COUNTERFACTUAL_FILE_SUFFIX + FILENAME_EXTENSION)[0]
...
def _score_individual(predictions_location, cf_dir_location):
    ...
    ufids = [f.rsplit(FILENAME_EXTENSION)[0]
             for f in ufids if f.lower().endswith(FILENAME_EXTENSION)]
