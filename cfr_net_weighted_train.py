...
tf.app.flags.DEFINE_string('mode', '2.0', """Proposed objective function (weighted). """)
...
def train(CFR, sess, propensity_step, train_step, D, I_valid, D_test, logfile, i_exp):
    ...
    for i in range(FLAGS.iterations):
        ...
        if i % FLAGS.output_delay == 0 or i==FLAGS.iterations-1:
            ...
            losses.append([obj_loss, f_error, cf_error, imb_err, valid_f_error, valid_imb, valid_obj])
            loss_str = str(i) + '\tObj: %.3f,\tF: %.3f,\tCf: %.3f,\tImb: %.2g,\tVal: %.3f,\tValImb: %.2g,\tValObj: %.2f,\ttime: %.2f' \
                        % (obj_loss, f_error, cf_error, imb_err, valid_f_error, valid_imb, valid_obj, elapsed_time)
