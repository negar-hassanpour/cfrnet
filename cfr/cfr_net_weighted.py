...
class cfr_net(object):
    ...
    def _build_graph(self, x, t, y_ , p_t, FLAGS, r_alpha, r_lambda, do_in, do_out, dims):
        ...
        ''' Construct the propensity network '''
        W = tf.Variable(tf.zeros([dim_in, 1]), name='W')
        b = tf.Variable(tf.zeros([1]), name='b')#-tf.log(1-1./p_t)
        sigma = tf.nn.sigmoid(tf.matmul(h_rep_norm, W) + b)
        pi_0 = tf.multiply(t, sigma) + tf.multiply(1-t, 1-sigma)
        cost = -tf.reduce_mean(tf.multiply(t, tf.log(sigma)) + tf.multiply(1-t, tf.log(1-sigma)), reduction_indices=1) \
            + tf.nn.l2_loss(W)
        self.W = W
        self.b = b
        self.pi_0 = pi_0
